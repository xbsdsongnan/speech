...
class ReLU(Layer):
    def forward(self, input):  
        output = None
        output = np.maximum(input, 0) #也可以利用Leaky ReLU，max(0.01x.x)
        
        return output

    def backward(self, input, output, d_output):

        in_diff = d_output * np.maximum(0, np.sign(input))
        return in_diff 

class FullyConnect(Layer):
    def __init__(self, in_dim, out_dim):
        self.w = np.random.randn(out_dim, in_dim) * np.sqrt(2.0 / in_dim)#A numpy array of weights,np.random.randn函数生成随机矩阵,np.sqrt 返回均方根
        self.b = np.zeros(out_dim) #A numpy array of biases
        self.dw = np.zeros((out_dim, in_dim)) #Gradient with respect to w
        self.db = np.zeros(out_dim) #Gradient with respect to b
    ...
    def forward(self, input):
        ...
        output = None
        mat_input = np.reshape(input, (input.shape[0], -1))#reshape(m,-1)改变维度为m行、1列
        output = np.dot(mat_input, self.w.T) + self.b
        return output
    ...
    def backward(self, input, output, d_output):
        ...
        in_diff = np.reshape(in_diff, input.shape)
        mat_input = np.reshape(input, (input.shape[0], -1))#转为二维向量
        self.dw = np.dot(mat_input.T, d_output).T
        #print(self.dw.shape)
...
class Softmax(Layer):
    def forward(self, input):
        row_max = input.max(axis=1).reshape(input.shape[0], 1) #scores -= np.max(scores, axis = 1)[:, np.newaxis]
        x = input - row_max
        return np.exp(x) / np.sum(np.exp(x), axis=1).reshape(x.shape[0], 1) #softmaxi = np.exp( score(yi)) / np.sum(np.exp(score)) 
...
def main():
    #np.random.seed() #按照默认的一个序列生成随机数
    np.random.seed(23333) #在所设置的23333范围内调用random()模块生成随机数，如果再次启动，它则按照之前的序列从头开始生成随机数，两次生成的随机序列相同。
    # We splice the raw feat with left 5 frames and right 5 frames
    # So the input here is 39 * (5 + 1 + 5) = 429
    dnn = DNN(429, 256, 128, 1)
